...
export const API_CONFIG = {
  // Firebase Functions URLs (продакшн) - теперь используем относительный путь /api для prod
  ...
  get BASE_URL() {
    ...
    if (!isMainAppDev) {
      // Используем относительный /api путь для продакшена (Firebase Hosting rewrites обработают)
