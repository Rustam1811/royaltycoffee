...
const UsersPage: React.FC = () => {
  ...
  async function openUser(u: ListedUser) {
    ...
    try {
      const [ordersRes, achRes] = await Promise.all([
        fetch(`/api/orders?action=get&userId=${u.id}`),
        fetch(`/api/promo?action=achievements&userId=${u.id}`)
      ]);
      if (ordersRes.ok) {
        const ord = await ordersRes.json();
        setUserOrders(Array.isArray(ord) ? ord : ord.orders || []);
      }
      if (achRes.ok) {
