...
// ===================================================================
const FormLabel = ({ icon, text }) => (<label className="flex items-center gap-2 text-sm font-semibold text-zinc-400 mb-2">
        {icon}
        {text}
    </label>);
const FormInput = ({ error = null, ...props }) => (<input {...props} className={`w-full p-3 bg-zinc-700/50 rounded-lg text-white placeholder-zinc-500 outline-none transition-all border border-zinc-700
            focus:bg-zinc-700 focus:ring-2 focus:ring-orange-500
            ${error ? 'ring-2 ring-red-500/80 border-transparent' : ''}`}/>);
const TimeSlotButton = ({ time, isActive, onClick }) => (<button type="button" onClick={onClick} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all border-2 ${isActive
...
const Booking = () => {
    ...
    if (showThankYou) {
        return (<div className="bg-zinc-900 min-h-screen flex flex-col items-center justify-center p-5 text-center font-sans">
    ...
    return (<div className="bg-zinc-900 min-h-screen font-sans">
            ...
            <main className="p-5">
                <form onSubmit={handleBooking} className="bg-zinc-800/50 border border-zinc-700/50 rounded-2xl shadow-xl p-6 space-y-6">
                    ...
                    <div className="pt-4">
                        <motion.button type="submit" disabled={loading} whileTap={{ scale: 0.95 }} className="w-full bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold text-lg py-4 rounded-xl shadow-lg shadow-orange-500/20 flex items-center justify-center gap-3 disabled:opacity-70">
