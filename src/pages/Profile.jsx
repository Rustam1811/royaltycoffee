...
import { TrophyIcon, TicketIcon, FireIcon, StarIcon, ChevronRightIcon, ArrowLeftOnRectangleIcon } from '@heroicons/react/24/solid';
// ===================================================================
//  ДАННЫЕ И ТИПЫ
// ===================================================================
...
//  КОМПОНЕНТЫ
// ===================================================================
const BalanceBar = ({ points, streak, lastVisitToday }) => (<div className="grid grid-cols-2 gap-4">
...
const NextRewardCard = ({ reward, stamps, stampsToReward, onClaim, isClaimable }) => {
    ...
    return (<div className="bg-white rounded-3xl shadow-xl p-6 flex flex-col items-center text-center border border-slate-200/80 relative overflow-hidden min-h-[340px]">
            <AnimatePresence mode="wait">
                {isClaimable ? (<motion.div key="claim" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="w-full flex flex-col h-full justify-between">
                        <div>
                        ...
                        <motion.button onClick={onClaim} whileTap={{ scale: 0.95 }} className="w-full bg-gradient-to-r from-emerald-500 to-green-500 text-white font-bold py-4 rounded-xl shadow-lg">
                            Получить награду
                        </motion.button>
                    </motion.div>) : (<motion.div key="progress" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full">
                        ...
                        <div className="relative w-36 h-36 my-4">
                            <svg className="w-full h-full" viewBox="0 0 100 100">
                                ...
                                <motion.circle cx="50" cy="50" r="45" className="stroke-orange-500" strokeWidth="10" fill="transparent" strokeLinecap="round" transform="rotate(-90 50 50)" initial={{ strokeDasharray: `0, 283` }} animate={{ strokeDasharray: `${progress * 2.83}, 283` }} transition={{ duration: 1.5, ease: "easeInOut" }}/>
                            </svg>
...
const AchievementIcon = ({ achievement }) => {
    ...
    return (<div className="flex flex-col items-center gap-2 text-center">
            <div className={`relative w-20 h-20 rounded-2xl flex items-center justify-center text-3xl transition-all duration-300 transform group-hover:-translate-y-1 ${achievement.unlocked ? rarityStyles[achievement.rarity] : "bg-slate-200 text-slate-400"}`}>
                {achievement.unlocked ? achievement.icon : "❓"}
                {achievement.isNew && (<span className="absolute -top-2 -right-2 block h-5 w-5">
...
const ConfettiExplosion = () => (<div className="fixed inset-0 z-[200] pointer-events-none">
        {[...Array(50)].map((_, i) => (<motion.div key={i} className="absolute w-2 h-4 bg-yellow-400 rounded-full" initial={{ x: '50vw', y: '50vh', opacity: 1 }} animate={{
...
const UltimateProfilePage = () => {
    ...
    return (<div className="bg-slate-50 min-h-screen font-sans text-slate-900">
            ...
            <main className="p-4 space-y-8 pb-28">
                ...
                <motion.section custom={2} initial="hidden" animate="visible" variants={sectionVariants}>
                    <NextRewardCard reward={profile.currentReward} stamps={profile.stamps} stampsToReward={profile.stampsToReward} onClaim={handleClaimReward} isClaimable={isRewardClaimable}/>
                </motion.section>
                ...
                <motion.section custom={3} initial="hidden" animate="visible" variants={sectionVariants}>
                    ...
                    <div className="flex space-x-4 overflow-x-auto -mx-4 px-4 pb-4" style={{ scrollbarWidth: 'none' }}>
                        {profile.achievements.map(ach => (<AchievementIcon key={ach.id} achievement={ach}/>))}
                    </div>
                </motion.section>
                ...
                <motion.section custom={4} initial="hidden" animate="visible" variants={sectionVariants}>
                    <h2 className="text-2xl font-bold text-slate-900 mb-3">Персональные квесты</h2>
                     <div className="space-y-3">
                        {profile.personalQuests.map(quest => (<div key={quest.id} className="bg-white p-4 rounded-2xl shadow-lg flex items-center gap-4 border border-slate-200/80">
