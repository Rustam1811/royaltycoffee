...
//  КОМПОНЕНТЫ
// ===================================================================
const ProfilePill = ({ name, avatar }) => (<div className="flex items-center gap-2 bg-white/60 backdrop-blur-lg rounded-full p-1 pr-3 border border-slate-200/80">
        <img src={avatar} alt={name} className="w-8 h-8 rounded-full object-cover"/>
        <span className="font-semibold text-slate-800 text-sm">Привет, {name}</span>
    </div>);
const StoryBubble = ({ pack, onClick, hasBeenViewed }) => (<div onClick={onClick} className="text-center w-[76px] flex-shrink-0 cursor-pointer group">
...
const StoryProgress = ({ stories, currentStoryIndex, onFinish }) => (<div className="absolute top-4 left-4 right-4 flex gap-1.5 z-20">
        {stories.map((story, index) => (<div key={story.id} className="h-1 flex-1 bg-white/30 rounded-full overflow-hidden">
...
const StoryViewer = ({ storyPacks, initialPackIndex, onClose }) => {
    ...
    const handleNext = useCallback(() => {
        if (currentStoryIndex < currentPack.stories.length - 1) {
            setCurrentStoryIndex(p => p + 1);
        }
        else if (currentPackIndex < storyPacks.length - 1) {
            setCurrentPackIndex(p => p + 1);
    ...
    return (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black z-[100] flex items-center justify-center">
...
const HomePage = () => {
    ...
    const [viewedPacks, setViewedPacks] = useState(new Set());
    const openStoryViewer = (packIndex) => {
        setInitialStoryIndex(packIndex);
        setIsViewerOpen(true);
        setViewedPacks(prev => new Set(prev).add(fakeStoriesData[packIndex].id));
    };
    return (<div className="bg-slate-100 min-h-screen font-sans">
            ...
            <main className="p-4 space-y-10 pb-28">
                ...
                <section>
                    <div className="flex space-x-4 overflow-x-auto -mx-4 px-4 pb-2" style={{ scrollbarWidth: 'none' }}>
                        {fakeStoriesData.map((pack, index) => (<StoryBubble key={pack.id} pack={pack} onClick={() => openStoryViewer(index)} hasBeenViewed={viewedPacks.has(pack.id)}/>))}
                    </div>
                </section>
                ...
                <section>
                     <div className="bg-white rounded-3xl shadow-xl border border-slate-200/60 p-6">
                        <div className="flex items-center gap-4">
                         ...
                         <motion.button whileTap={{ scale: 0.95 }} onClick={() => alert('Быстрый заказ!')} className="mt-4 w-full bg-slate-900 text-white font-bold py-3 px-5 rounded-xl flex items-center justify-center gap-2 shadow-lg hover:bg-slate-700 transition-colors">
                ...
                <section>
                     ...
                     <div className="flex space-x-4 overflow-x-auto -mx-4 px-4 pb-4" style={{ scrollbarWidth: 'none', scrollSnapType: 'x mandatory' }}>
                        {curatedListData.items.map(item => (<div key={item.id} className="flex-shrink-0 w-40" style={{ scrollSnapAlign: 'start' }}>
            ...
            <AnimatePresence>
                {isViewerOpen && (<StoryViewer storyPacks={fakeStoriesData} initialPackIndex={initialStoryIndex} onClose={() => setIsViewerOpen(false)}/>)}
            </AnimatePresence>
        </div>);
};
